...
func TestScanBufferExpandCappedByMax(t *testing.T) {
	sb := New(WithStartBufSize(16), WithMaxBufSize(24))
...
func TestIter(t *testing.T) {
	lines := []string{}
	r := strings.NewReader("a\nb\nc")
	s := New(WithStartBufSize(16))
	for res := range s.Iter(r) {
		lines = append(lines, string(res))
	}
	require.Equal(t, []string{"a", "b", "c"}, lines)
	require.NoError(t, s.IterErr())
}
...
func TestIterBreak(t *testing.T) {
	...
	require.Equal(t, []string{"a", "b", "stop"}, lines)
	require.NoError(t, s.IterErr())
}
